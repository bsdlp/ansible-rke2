variable "hostname_prefix" { default = "node" }
...
resource "libvirt_volume" "os_image" {
  count  = var.vm_count
  name   = "${var.hostname_prefix}-os_image-${count.index}"
...
resource "libvirt_volume" "os_image_resized" {
  count          = var.vm_count
  name           = "${var.hostname_prefix}-os_image_resized-${count.index}"
  pool           = var.libvirt_pool
...
resource "libvirt_cloudinit_disk" "commoninit" {
  count     = var.vm_count
  name      = "${var.hostname_prefix}-${count.index}-commoninit.iso"
  pool      = var.libvirt_pool
...
data "template_file" "user_data" {
  ...
  vars = {
    ...
    fqdn     = "${var.hostname_prefix}-${count.index}"
    sshKey   = "${var.sshKey}"
  }
}
...
output "macs" {
  value = flatten(libvirt_domain.node.*.network_interface.0.mac)
}
